...
DEFINE_uint64(rtk_trajectory_forward, 800,
              "The number of points to be included in RTK trajectory " // 轨道
...
DEFINE_bool(enable_reference_line_stitching, true,
            "Enable stitching reference line, which can reducing computing "  // 启用缝合参考线，减少计算 
            "time and improve stability");
DEFINE_double(look_forward_extend_distance, 50,
              "The step size when extending reference line.");          // 延伸参照线时的步长
DEFINE_double(reference_line_stitch_overlap_distance, 20,
              "The overlap distance with the existing reference line when "  //与现有参考线重合的距离
              "stitching the existing reference line");

DEFINE_bool(enable_smooth_reference_line, true,
            "enable smooth the map reference line");  // 启用平滑参考线

DEFINE_bool(prioritize_change_lane, false,
            "change lane strategy has higher priority, always use a valid "  // “更改车道策略具有更高的优先级，请始终使用有效的”
                                                                             // “如果存在车道路径，请更改该路径”
